...
import cmsFakerFactory from "./cmsFakerFactory";
...
const CmsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = cmsFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("cms").create(elem));
...
export default connect(mapState, mapDispatch)(CmsFakerDialogComponent);
